...
public class FileFilter {
    //用来存放过滤后的文件
    private static ArrayList<File> resultFiles = new ArrayList<>();
    private static FileInputStream stream = null;
    private static StringBuilder content = new StringBuilder();
    private static final String rex = "(?<=\")(\\s*select|\\s*join).*?(?=\")";
    private static final String sqlJoin = "join";
    private static final String sqlOrg = "(=\\s*)[a-z]*(.OrgId)";
    private static Pattern p = Pattern.compile(rex);
